...
（golden/*.txt，在 pi TS mono-repo 中运行 dump 脚本生成后拷入）逐字符一致。

golden 文件缺失时测试跳过（不是失败）——需要先在装有 pi mono-repo 依赖的
机器上生成 golden 并拷入 golden/（见 README.md）。
"""
...
# 与 TS 侧生成 golden 时的 PI_PACKAGE_DIR 保持一致，保证 "Pi documentation"
# 段中的路径两边相同。
...
def test_system_prompt_matches_ts_golden(
    ...
    if not golden.exists():
        pytest.skip("TS golden 缺失：需在 pi TS mono-repo 中生成后拷入 golden/（见 README.md）")
