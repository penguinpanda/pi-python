"""v4 Session 内存模型 conformance 测试。

移植 TS `packages/agent/src/harness/session/testing/conformance.ts`
的用例，backend 为 `InMemorySessionRepo`（M0 验收）。
"""
...
def operation_started(record_id: str, lane: str, kind: str) -> dict:
    if kind == "run":
...
class TestEntriesAndLanes:
    @pytest.mark.asyncio
    async def test_assigns_parents_and_one_sequence(self, repo):
    ...
    @pytest.mark.asyncio
    async def test_rejects_duplicate_ids_without_changing_state(self, repo):
        session = await repo.create({"id": "session"})
    ...
    @pytest.mark.asyncio
    async def test_isolates_lanes_while_sharing_the_tree(self, repo):
        session = await repo.create({"id": "session"})
        root = await session.append_entry(
            {"type": "message", "id": "root", "message": create_user_message("root")},
            "main",
    ...
    @pytest.mark.asyncio
    async def test_validates_lane_lifecycle_and_targets(self, repo):
        session = await repo.create({"id": "session"})
    ...
    @pytest.mark.asyncio
    async def test_binds_lane_views_without_caching_leaves(self, repo):
        session = await repo.create({"id": "session"})
    ...
    @pytest.mark.asyncio
    async def test_appends_provisioned_entries_with_existing_ids(self, repo):
        session = await repo.create({"id": "session"})
    ...
    @pytest.mark.asyncio
    async def test_persists_tool_result_termination_decisions(self, repo):
        session = await repo.create({"id": "session"})
        entry = await session.append_entry(
            {
                "type": "message",
    ...
    @pytest.mark.asyncio
    async def test_linearizes_concurrent_writes_across_two_lanes(self, repo):
        session = await repo.create({"id": "session"})
        root = await session.append_entry(
            {"type": "message", "id": "root", "message": create_user_message("root")},
            "main",
        ...
        async def write(entry_id: str, lane: str) -> dict:
            entry = await session.append_entry(
...
class TestRecordsAndLog:
    @pytest.mark.asyncio
    async def test_commits_records_and_lane_moves_as_separate_mutations(self, repo):
        session = await repo.create({"id": "session"})
        root = await session.append_entry(
            {"type": "message", "id": "root", "message": create_user_message("root")},
            "main",
    ...
    @pytest.mark.asyncio
    async def test_keeps_lane_names_permanent_with_their_recovery_records(self, repo):
        session = await repo.create({"id": "session"})
    ...
    @pytest.mark.asyncio
    async def test_persists_queue_cancellation_without_consuming_target(self, repo):
        session = await repo.create({"id": "session"})
    ...
    @pytest.mark.asyncio
    async def test_filters_records_by_lane_type_run_sequence_and_order(self, repo):
        session = await repo.create({"id": "session"})
    ...
    @pytest.mark.asyncio
    async def test_filters_operation_starts_by_operation_kind(self, repo):
        session = await repo.create({"id": "session"})
    ...
    @pytest.mark.asyncio
    async def test_tracks_and_enforces_one_open_operation_per_lane(self, repo):
        session = await repo.create({"id": "session"})
    ...
    @pytest.mark.asyncio
    async def test_earlier_finish_does_not_close_later_start(self, repo):
        session = await repo.create({"id": "session"})
        await session.append_record(
            {
                "type": "operation_finished",
    ...
    @pytest.mark.asyncio
    async def test_scopes_open_operations_by_lane_and_limit(self, repo):
        session = await repo.create({"id": "session"})
        await session.create_lane("thread", None)
    ...
    @pytest.mark.asyncio
    async def test_returns_immutable_open_operation_records(self, repo):
        session = await repo.create({"id": "session"})
...
class TestQueriesAndFacts:
    @pytest.mark.asyncio
    async def test_rejects_invalid_queries_before_empty_reads(self, repo):
        ...
        await rejects_with_code(
            session.find_open_operations("main", {"limit": -1}), "invalid_query"
        )
        await rejects_with_code(session.get_log({"afterSeq": -1}), "invalid_query")

    @pytest.mark.asyncio
    async def test_supports_bounded_filtered_and_cursor_queries(self, repo):
        session = await repo.create({"id": "session"})
        await session.append_entry(
            {"type": "message", "id": "root", "message": create_user_message("root")},
            "main",
        ...
        assert entry_ids(
            await session.find_entries(
                {"order": "oldestFirst", "cursor": {"afterSeq": 2}, "limit": 2}
            )
        ) == ["compact", "new-note"]
        ...
        assert entry_ids(
            await session.find_entries_on_branch(
                {"start": "tail", "customType": "note", "limit": 1}
            )
        ) == ["new-note"]
        assert entry_ids(
            await session.find_entries_on_branch(
    ...
    @pytest.mark.asyncio
    async def test_keeps_latest_value_facts_and_computes_stats(self, repo):
        session = await repo.create({"id": "session"})
...
class TestValidationAndImmutability:
    @pytest.mark.asyncio
    async def test_returns_immutable_copies_from_reads(self, repo):
    ...
    @pytest.mark.asyncio
    async def test_rejects_non_json_entries_before_storage_mutation(self, repo):
        session = await repo.create({"id": "session"})
        cyclic: dict = {}
        cyclic["self"] = cyclic
    ...
    @pytest.mark.asyncio
    async def test_rejects_non_json_records_before_storage_mutation(self, repo):
        session = await repo.create({"id": "session"})
...
class TestRepositoryAndForks:
    @pytest.mark.asyncio
    async def test_creates_lists_and_opens_sessions(self, repo):
    ...
    @pytest.mark.asyncio
    async def test_deletes_sessions_idempotently(self, repo):
        session = await repo.create({"id": "one"})
        metadata = await session.get_metadata()
    ...
    @pytest.mark.asyncio
    async def test_forks_one_branch_with_selected_facts_and_no_records(self, repo):
    ...
    @pytest.mark.asyncio
    async def test_forks_a_complete_tree_with_lanes_and_facts(self, repo):
        source = await repo.create({"id": "source"})
        root = await source.append_message(create_user_message("root"))
    ...
    @pytest.mark.asyncio
    async def test_forks_before_an_entry_without_modifying_source(self, repo):
        source = await repo.create({"id": "source"})
        root = await source.append_message(create_user_message("root"))
        ...
        assert entry_ids(await before_default_target.find_entries({"order": "oldestFirst"})) == [
            root
        ]
    ...
    @pytest.mark.asyncio
    async def test_validates_the_default_fork_target(self, repo):
