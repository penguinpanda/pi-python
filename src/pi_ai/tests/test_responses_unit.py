...
class TestResponsesNormalizeToolCallId:
    ...
    def test_pipe_id_kept_for_allowed_provider(self):
        ...
        source = {'role': 'assistant', 'provider': 'openai', 'api': 'openai-responses', 'model': 'gpt-5'}
    ...
    def test_foreign_item_id_hashed_to_fc(self):
        ...
        source = {'role': 'assistant', 'provider': 'deepseek', 'api': 'openai-completions', 'model': 'deepseek-chat'}
...
class TestResponsesReasoningReplay:
    ...
    def test_thinking_signature_replayed_as_reasoning_item(self):
        ...
        messages = [{'role': 'assistant', 'content': [
            {'type': 'thinking', 'thinking': 'think', 'thinking_signature': json.dumps(reasoning_item)},
        ...
        assert result[1]['content'] == [{'type': 'output_text', 'text': 'Answer', 'annotations': []}]
    ...
    def test_text_signature_id_and_phase_replayed(self):
        model = _make_model()
        messages = [{'role': 'assistant', 'content': [
            {'type': 'text', 'text': 'Answer', 'text_signature': '{"v":1,"id":"msg_abc","phase":"final_answer"}'},
        ], 'api': 'openai-responses', 'provider': 'openai', 'model': 'gpt-4o'}]
        result = _to_responses_input(messages, None, model)
    ...
    def test_function_call_item_id_kept_same_model(self):
        ...
        messages = [{'role': 'assistant', 'content': [
            {'type': 'toolCall', 'id': 'call_1|fc_123', 'name': 'bash', 'raw_arguments': '', 'arguments': {'command': 'ls'}},
        ], 'api': 'openai-responses', 'provider': 'openai', 'model': 'gpt-4o'}]
        result = _to_responses_input(messages, None, model)
        assert result[0] == {
            'type': 'function_call',
    ...
    def test_function_call_item_id_dropped_different_model(self):
        ...
        model = _make_model(model_id='gpt-5', provider='openai', api='openai-responses')
        messages = [{'role': 'assistant', 'content': [
            {'type': 'toolCall', 'id': 'call_1|fc_123', 'name': 'bash', 'raw_arguments': '', 'arguments': {}},
        ], 'api': 'openai-responses', 'provider': 'openai', 'model': 'gpt-4o'}]
        result = _to_responses_input(messages, None, model)
        assert result[0] == {
            'type': 'function_call',
    ...
    def test_tool_result_pipe_call_id_split(self):
        model = _make_model()
        messages = [{'role': 'toolResult', 'tool_call_id': 'call_1|fc_123', 'tool_name': 'bash',
    ...
    def test_tool_result_empty_output_placeholder(self):
        model = _make_model()
        messages = [{'role': 'toolResult', 'tool_call_id': 'call_1', 'tool_name': 'bash',
...
class TestResponsesStreamReasoningCapture:
    ...
    async def test_tool_call_stores_pipe_id(self):
        ...
        events = [
            _event("response.output_item.added", item=SimpleNamespace(
                type="function_call", call_id="call_1", id="fc_abc", name="get_weather",
            )),
            _event("response.function_call_arguments.delta", delta='{"city":'),
            _event("response.function_call_arguments.delta", delta='"Beijing"}'),
            _event("response.function_call_arguments.done"),
