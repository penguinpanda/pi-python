...
class ConfigSelectorComponent(Widget):
    ...
    def handle_key(self, key: Key) -> bool:
        if key.name == "up":
            self._selected = (self._selected - 1) % max(1, len(self._entries))
            self.refresh()
            return True
        if key.name == "down":
            self._selected = (self._selected + 1) % max(1, len(self._entries))
        ...
        if key.name in ("enter", " "):
            if self._entries:
                ...
                self._on_toggle(entry, not bool(entry.get("enabled")))
    ...
    def render(self, width: int, height: int) -> list[Line]:
        lines = [line_from_text("Local resources", width)]
        for index, entry in enumerate(self._entries[: max(1, height - 1)]):
            marker = ">" if index == self._selected else " "
            check = "[x]" if entry.get("enabled") else "[ ]"
            label = f"{marker} {check} {entry.get('resource_type', '')}: {entry.get('path', '')}"
            lines.append(line_from_text(label, width))
        while len(lines) < height:
            lines.append(blank_line(width))
        return lines

    def content_size(self) -> tuple[int, int]:
        return (80, min(1 + len(self._entries), 16))
